...
{
    static const KeyVerbatim &allowed = "_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    if( key.empty() )
        return false;
    for( auto &symbol : key )
    {
...
bool Object::push( const KeyVerbatim &key, const Item &item )
{
    Item copy = item;
    return push( key, std::move( copy ) );
}

bool Object::push( const KeyVerbatim &key, Item &&item )
{
    makeException( verifyVerbatim( key ) );
    if( item.is<Null>() )
        return items.erase( key );
    return items.emplace( key, item ).second;
}
...
bool Array::push( const Item &item )
{
    Item copy = item;
    return push( std::move( copy ) );
}

bool Array::push( Item &&item )
{
    if( !item.is<Null>() )
...
bool Array::push( const KeyNumeric &key, const Item &item )
{
    Item copy = item;
    return push( key, std::move( copy ) );
}

bool Array::push( const KeyNumeric &key, Item &&item )
{
    if( key >= size() )
    {
        if( item.is<Null>() )
